function [Zsave, dt_save] = Hopf_Delays_SquareStim(f,K,MD,SynDelay,sig,input_amp,t_pert,pulse_dur,Nodes_to_stim, varargin)
...
% Adapted by Mariana Henriques mariana.m.henriques@tecnico.ulisboa.pt
% to include square pulse stimulation
%----------------------------------------------------------------------
...
for t=dt:dt:t_prev+tmax %t=1:tmax/dt when adding stimulation  % We only start saving after t_prev
    ...
    if t >= t_pert
        % At a given time, start square pulse perturbation 
        if t < t_pert + pulse_dur
            Perturb = input_amp*ones(90,1);
        end
    end

    Znow(Nodes_to_stim) = Znow(Nodes_to_stim) + Perturb(Nodes_to_stim); 
